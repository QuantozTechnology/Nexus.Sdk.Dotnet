...
public interface IAccountsFacade
{
    ...
    /// <returns>The Nexus account that is created</returns>
    public Task<AccountResponse> CreateOnAlgorandAsync(string customerCode, string publicKey, string? customerIPAddress = null, string? customName = null);
    ...
    /// <param name="customName">Optional custom name for account</param>
    /// <returns>A transaction that needs to be signed using the private key that matches the provided public key</returns>
    public Task<SignableResponse> CreateOnAlgorandAsync(string customerCode, string publicKey, IEnumerable<string> allowedTokens, string? customerIPAddress = null, string? customName = null);
