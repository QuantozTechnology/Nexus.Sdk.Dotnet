...
public class OperationsFacade : TokenServerFacade, IOperationsFacade
{
    ...
    {
        return await _provider.CreatePayoutAsync(accountCode, tokenCode, amount, pm, memo, message, paymentReference, customerIPAddress, blockchainTransactionId, nonce, bankAccountNumber, data);
    }
    ...
    {
        return await _provider.UpdateOperationStatusAsync(operationCode, status, comment, customerIPAddress, paymentReference, data);
    }
}
